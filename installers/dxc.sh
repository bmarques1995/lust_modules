...
if [ $passed ]; then
    if [ -n "$2" ]; then
        cmake -S ./modules/dxc -B ./lin_deps/dxc -G Ninja -C ./modules/dxc/cmake/caches/PredefinedParams.cmake -DCMAKE_BUILD_TYPE=$1 -DCMAKE_C_COMPILER=clang -DCMAKE_CXX_COMPILER=clang++ -DCMAKE_INSTALL_PREFIX=$2
